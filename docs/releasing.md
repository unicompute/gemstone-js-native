...
malformed, pathful, or duplicated. The checksum writer excludes
`SHA256SUMS.txt` itself from artifact suffix matches, so broad suffix checks
cannot include the manifest as an artifact target.
